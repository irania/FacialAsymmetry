...
# Initialize video capture and facial landmark detector
...
out = cv2.VideoWriter('out2.mp4', fourcc, fps, (frame_width, frame_height))

barycentric_matrices = []
while cap.isOpened():
    ret, original_frame = cap.read()
    if not ret:
        break

    frame = original_frame.copy()  # Create a copy of the frame for displaying text
    face_obj = Face(frame)
    ...
    for face in faces:
        ...
        for n in range(0, 58):
            ...
            # Add the matrix for the current frame to the list
            barycentric_matrices.append(frame_matrix)
        ...
        cv2.putText(frame, f"Barycentric Asymmetry Index: {asymmetry_index:.2f}", (10, 30), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 255, 0), 2)
        cv2.putText(frame, f"Our Asymmetry Index: {asymmetry_norm_whole:.2f}", (10, 70), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 255, 0), 2)
        cv2.putText(frame, f"Our Vertically Asymmetry Index: {asymmetry_vertically_whole:.2f}", (10, 110), cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 255, 0), 2)
